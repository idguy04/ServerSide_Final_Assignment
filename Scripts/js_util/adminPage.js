const { theMoviesDbAPIKey } = require("./apiKeys.js");

//===== DOCUMENT READY =====//

$(document).ready(function () {
  // init global data
  key = theMoviesDbAPIKey;
      ...
      columns: [
        ...
        { data: "Birthyear" },
        {
          render: function (data, type, row, meta) {
            ...
            return (
              "<input class='editableCB' disabled type='checkbox' " +
              isChecked +
              ">"
            );
          },
...
function updateUser_success(user_id) {
  showAlert(
    "success",
    "Update Success!",
    "User ID: " + user_id + " updated successfuly!",
    ""
  );
  if (!changeFlag) {
    clearLastUserEdit();
      ...
      columns: [
        {
          render: function (data, type, row, meta) {
            ...
            viewTvBtn =
              "<button onclick='fillTvDescPH(this.id)' id='" +
              row.Show_id +
              "' type='button' class = 'viewOverview tvBtn btn btn-success' " +
              dataTV +
              "> Overview </button>"; //overview buttons

            viewEpisodeDDL =
              "<select onclick='getAdminEpisodes(this.id, this.value)' id='" +
              row.Show_id +
              "' class='viewSeasons btn'>"; //onchange event cant re-render the already selected item, once clicked again. therefore we want to use the onclick event instead.
...
function getAdminEpisodes(show_id, season_num) {
  ajaxCall(
    "GET",
    "../api/Episodes?show_id=" + show_id + "&season_num=" + season_num,
    "",
    getAdminEpisodes_success,
    getAdminEpisodes_error
  );
}
...
function getAdminEpisodes_success(episodesData) {
  ...
  if (episodesTbl == null) renderEpisodesTable(episodes);
  // else redraw the table from the last season of the tvshow that was rendered.
  else redrawEpisodesTbl(episodesTbl, episodes);
...
function drawEpisodesHeader() {
  let $header = $("<h2>").text(
    episodes[0].Show_name + " - Season " + episodes[0].Season_num
  );
  let div = $("<div class='header'>").append($header);
...
function markSelected(btn) {
  if (lastRowSelected != null) lastRowSelected.classList.remove("selected"); // remove the last row selected(last row = DOM element)
