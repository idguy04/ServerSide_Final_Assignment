...
function getTV_SuccessCB(tv) {
  ...
  ajaxCall(
    ...
    seasonsApiCall,
    "",
    getEverySeason_SuccessCB,
    getEverySeason_ErrorCB
  );
...
function getEverySeason_SuccessCB(season) {
  renderSeason(season);
  let seasonsApiCall =
    apiPrefix + "/season/" + (season.season_number + 1) + apiSuffix;
...
function getSpecificSeason_ErrorCB(err) {
  showAlert(
    "error",
    "Oops...",
    "failed to get this specific season!",
    "try another one..."
  );
}
...
function postEpisode_SuccessCB() {
  showAlert(
    "success",
    "YAY!",
    "Episode inserted successfuly!",
    'You can view it in "view" page...'
  );
...
function wireSeasonCSS() {
  $("#ph div").hover(
    // add red shadow when hovering over a specific season
    function () {
  ...
  $("#ph div").click(function () {
    // assign season clicking a specific season
    ...
    ajaxCall(
      ...
      apiCall,
      "",
      getSpecificSeason_SuccessCB,
      getSpecificSeason_ErrorCB
    );
  });
}
...
function renderEpisodes(season) {
  ...
  for (let i = 0; i < episodes.length; i++) {
    ...
    let button =
      "<button onclick='postEpisode(" +
      i +
      ",this)' class='addBTN button'>Save</button>";
...
function renderDetails() {
  ...
  let detailsDiv =
    "<div id='detailsPH'>" + showHeader + overview + statsPH + "</div>";

  let img =
    "<div><img src='" + imagePath + chosenTvShow.poster_path + "'></div>";
...
function renderRating() {
  ...
  $("#statsPH").append(
    "<h5>Rating</h5><div class='rating'>" + ratingPercentage + "%</div>"
  );
}
...
function renderTrailer() {
  ...
  str +=
    "<img href='#media-popup' data-media='//www.youtube.com/embed/" +
    trailerKey +
    "' src='../css/images/youtube.png'/>";
